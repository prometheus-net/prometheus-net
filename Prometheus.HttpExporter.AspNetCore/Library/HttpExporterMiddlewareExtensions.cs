...
namespace Prometheus.HttpExporter.AspNetCore.Library
{
    public static class HttpExporterMiddlewareExtensions
    {
        ...
        public static IApplicationBuilder UseHttpExporter(this IApplicationBuilder app, HttpMiddlewareExporterOptions options = null)
        {
            ...
            if (options.RequestCount.Enabled) app.UseMiddleware<HttpRequestCountMiddleware>(options.RequestCount.Counter);
            if (options.RequestDuration.Enabled) app.UseMiddleware<HttpRequestDurationMiddleware>(options.RequestDuration.Histogram);
